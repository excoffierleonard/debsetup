...
install_defaultrepo_tools() {
    ...
    apt install -y sudo neovim git curl wget mc ffmpeg tmux btop ncdu iftop rclone rsync tree ufw neofetch cpufetch cmatrix fzf exa tldr ripgrep qrencode certbot npm zip unzip htop zsh zsh-syntax-highlighting zsh-autosuggestions jq shellcheck ninja-build gettext libtool libtool-bin autoconf automake cmake g++ pkg-config doxygen pip python3-venv libxml2-dev r-base r-base-dev bat
}
